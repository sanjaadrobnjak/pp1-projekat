// generated with ast extension for cup
// version 0.8
// 20/0/2025 13:21:54


package rs.ac.bg.etf.pp1.ast;

public abstract class VisitorAdaptor implements Visitor { 

    public void visit(Mulop Mulop) { }
    public void visit(MinusMaybeTerm MinusMaybeTerm) { }
    public void visit(Relop Relop) { }
    public void visit(RelopExprMaybe RelopExprMaybe) { }
    public void visit(VarDeclarat VarDeclarat) { }
    public void visit(ParenActParsMaybe ParenActParsMaybe) { }
    public void visit(CondTermList CondTermList) { }
    public void visit(DesignatorStmtMaybe DesignatorStmtMaybe) { }
    public void visit(Addop Addop) { }
    public void visit(CondDesignatorStmtMaybe CondDesignatorStmtMaybe) { }
    public void visit(MaybeExpr MaybeExpr) { }
    public void visit(ActParsMaybe ActParsMaybe) { }
    public void visit(Factor Factor) { }
    public void visit(CondFactList CondFactList) { }
    public void visit(DeclList DeclList) { }
    public void visit(DesOrExpr DesOrExpr) { }
    public void visit(MethodName MethodName) { }
    public void visit(MulFactorList MulFactorList) { }
    public void visit(FormParsList FormParsList) { }
    public void visit(Statements Statements) { }
    public void visit(ElseStatement ElseStatement) { }
    public void visit(ExprList ExprList) { }
    public void visit(VarsList VarsList) { }
    public void visit(Expr Expr) { }
    public void visit(DesignatorStatement DesignatorStatement) { }
    public void visit(ConstsList ConstsList) { }
    public void visit(Statement Statement) { }
    public void visit(ConstVarDecls ConstVarDecls) { }
    public void visit(VarDecl VarDecl) { }
    public void visit(ExprOrActPars ExprOrActPars) { }
    public void visit(AddTermList AddTermList) { }
    public void visit(ConstDecl ConstDecl) { }
    public void visit(ActParsParenMaybe ActParsParenMaybe) { }
    public void visit(MethodDeclList MethodDeclList) { }
    public void visit(NumConstMaybe NumConstMaybe) { }
    public void visit(Consts Consts) { }
    public void visit(ActParsOrExpr ActParsOrExpr) { }
    public void visit(FormPars FormPars) { }
    public void visit(IdentExprList IdentExprList) { }
    public void visit(Label Label) { visit(); }
    public void visit(NoDesignatorStmtComma NoDesignatorStmtComma) { visit(); }
    public void visit(YesDesignatorStmtComma YesDesignatorStmtComma) { visit(); }
    public void visit(NoConditionDesignatorStmt NoConditionDesignatorStmt) { visit(); }
    public void visit(YesConditionDesignatorStmt YesConditionDesignatorStmt) { visit(); }
    public void visit(NoNumConstComma NoNumConstComma) { visit(); }
    public void visit(YesNumConstComma YesNumConstComma) { visit(); }
    public void visit(NoExpression NoExpression) { visit(); }
    public void visit(YesExpression YesExpression) { visit(); }
    public void visit(LessOrEqualOper LessOrEqualOper) { visit(); }
    public void visit(LessOper LessOper) { visit(); }
    public void visit(GreaterOrEqualOper GreaterOrEqualOper) { visit(); }
    public void visit(GreaterOper GreaterOper) { visit(); }
    public void visit(NotEqualOper NotEqualOper) { visit(); }
    public void visit(EqualOper EqualOper) { visit(); }
    public void visit(NoRelopExpr NoRelopExpr) { visit(); }
    public void visit(YesRelopExpr YesRelopExpr) { visit(); }
    public void visit(CondFact CondFact) { visit(); }
    public void visit(NoConditionFactors NoConditionFactors) { visit(); }
    public void visit(ConditionFactors ConditionFactors) { visit(); }
    public void visit(CondTerm CondTerm) { visit(); }
    public void visit(NoConditionTerms NoConditionTerms) { visit(); }
    public void visit(ConditionTerms ConditionTerms) { visit(); }
    public void visit(Condition Condition) { visit(); }
    public void visit(NoElseStatement NoElseStatement) { visit(); }
    public void visit(YesElseStatement YesElseStatement) { visit(); }
    public void visit(ActParsParenYes ActParsParenYes) { visit(); }
    public void visit(BracketExpressionYes BracketExpressionYes) { visit(); }
    public void visit(NoParenActPars NoParenActPars) { visit(); }
    public void visit(YesParenActPars YesParenActPars) { visit(); }
    public void visit(ParenExpression ParenExpression) { visit(); }
    public void visit(NewTypeExprActPars NewTypeExprActPars) { visit(); }
    public void visit(BooleanConstantt BooleanConstantt) { visit(); }
    public void visit(CharacterConstantt CharacterConstantt) { visit(); }
    public void visit(NumberConstantt NumberConstantt) { visit(); }
    public void visit(DesActPars DesActPars) { visit(); }
    public void visit(ModOper ModOper) { visit(); }
    public void visit(DivOper DivOper) { visit(); }
    public void visit(MulOper MulOper) { visit(); }
    public void visit(NoMulopFactor NoMulopFactor) { visit(); }
    public void visit(MulopFactor MulopFactor) { visit(); }
    public void visit(Term Term) { visit(); }
    public void visit(MinusOper MinusOper) { visit(); }
    public void visit(PlusOper PlusOper) { visit(); }
    public void visit(NoAddopTerm NoAddopTerm) { visit(); }
    public void visit(AddopTerm AddopTerm) { visit(); }
    public void visit(NoMinus NoMinus) { visit(); }
    public void visit(YesMinus YesMinus) { visit(); }
    public void visit(DesMapDes DesMapDes) { visit(); }
    public void visit(MinusTermAddTerm MinusTermAddTerm) { visit(); }
    public void visit(NoExpressions NoExpressions) { visit(); }
    public void visit(Expressions Expressions) { visit(); }
    public void visit(ActPars ActPars) { visit(); }
    public void visit(NoActParameters NoActParameters) { visit(); }
    public void visit(ActParameters ActParameters) { visit(); }
    public void visit(Decr Decr) { visit(); }
    public void visit(Incr Incr) { visit(); }
    public void visit(ParenActPars ParenActPars) { visit(); }
    public void visit(AssignExpr AssignExpr) { visit(); }
    public void visit(Assignop Assignop) { visit(); }
    public void visit(Setop Setop) { visit(); }
    public void visit(NoIdentExpr NoIdentExpr) { visit(); }
    public void visit(BracketExpr BracketExpr) { visit(); }
    public void visit(DotIdent DotIdent) { visit(); }
    public void visit(Designator Designator) { visit(); }
    public void visit(DesAssDesSetDes DesAssDesSetDes) { visit(); }
    public void visit(DesStmOr DesStmOr) { visit(); }
    public void visit(BraceStatements BraceStatements) { visit(); }
    public void visit(WhileLoop WhileLoop) { visit(); }
    public void visit(PrintExprNumConst PrintExprNumConst) { visit(); }
    public void visit(ReadDesignatorStm ReadDesignatorStm) { visit(); }
    public void visit(ReturnExpr ReturnExpr) { visit(); }
    public void visit(ContinueStm ContinueStm) { visit(); }
    public void visit(BreakStm BreakStm) { visit(); }
    public void visit(IfCondElseStmt IfCondElseStmt) { visit(); }
    public void visit(DesignatorStmt DesignatorStmt) { visit(); }
    public void visit(NoStmts NoStmts) { visit(); }
    public void visit(Stmts Stmts) { visit(); }
    public void visit(Type Type) { visit(); }
    public void visit(NoFormParameters NoFormParameters) { visit(); }
    public void visit(FormParameters FormParameters) { visit(); }
    public void visit(NoFormParams NoFormParams) { visit(); }
    public void visit(FormParams FormParams) { visit(); }
    public void visit(NoVariableDeclarationss NoVariableDeclarationss) { visit(); }
    public void visit(VariableDeclarationss VariableDeclarationss) { visit(); }
    public void visit(MethodDecl MethodDecl) { visit(); }
    public void visit(NoMethodDeclarations NoMethodDeclarations) { visit(); }
    public void visit(MethodDeclarations MethodDeclarations) { visit(); }
    public void visit(VoidName VoidName) { visit(); }
    public void visit(TypeName TypeName) { visit(); }
    public void visit(MethodSignature MethodSignature) { visit(); }
    public void visit(NoVariables NoVariables) { visit(); }
    public void visit(Variables Variables) { visit(); }
    public void visit(NoBracket NoBracket) { visit(); }
    public void visit(Bracket Bracket) { visit(); }
    public void visit(VarDeclarations VarDeclarations) { visit(); }
    public void visit(NoConstants NoConstants) { visit(); }
    public void visit(Constants Constants) { visit(); }
    public void visit(CharConstSingle CharConstSingle) { visit(); }
    public void visit(BoolConstSingle BoolConstSingle) { visit(); }
    public void visit(NumConstSingle NumConstSingle) { visit(); }
    public void visit(ConstDeclarations ConstDeclarations) { visit(); }
    public void visit(VarDecls VarDecls) { visit(); }
    public void visit(ConstDecls ConstDecls) { visit(); }
    public void visit(NoDeclarations NoDeclarations) { visit(); }
    public void visit(Declarations Declarations) { visit(); }
    public void visit(Program Program) { visit(); }


    public void visit() { }
}
